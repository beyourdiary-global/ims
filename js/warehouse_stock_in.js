...
function renderList(input, hiddenInput, options) {
	...
	if (keyword === '' || input.hasAttribute('readonly') || input.hasAttribute('disabled')) return;
...
function updateRowActionButtons() {
	...
	document.querySelectorAll('#stockInItemBody tr').forEach(function (row, idx) {
		...
		if (idx === 0) {
			actionCell.innerHTML = '<button class="mt-1" id="action_menu_btn" type="button" onclick="AddStockInRow()"><i class="fa-regular fa-square-plus fa-xl" style="color:#37c22e"></i></button>';
		} else {
			actionCell.innerHTML = '<button class="mt-1 remove-stock-row" id="action_menu_btn" type="button" onclick="RemoveStockInRow(this)"><i class="fa-regular fa-trash-can fa-xl" style="color:#ff0000"></i></button>';
		}
	});
}
