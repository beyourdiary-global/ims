...
$(document).ready(function ($) {
    ...
    $('a[name="exportBtn"]').on("click", function () {
        ...
        $('#stripe_trans_backup_table').DataTable().rows({ search: "applied", page: "current" }).nodes().to$().each(function () {
    ...
    function updateCheckboxesOnOtherPages(isChecked) {
        ...
        var cells = $('#stripe_trans_backup_table').DataTable().rows({ page: "current" }).nodes();
