...
if (!empty($checkboxValues)) {
    if (!isActionAllowed("Export", $pinAccess)) {
        ...
        ob_end_clean();
        echo "<script>alert('You do not have permission to export this page.'); location.href='" . $SITEURL . "/product/package_table.php';</script>";
        exit;
    }
                                    ...
                                    <td class="btn-container">
                                        ...
                                        <?php if (isActionAllowed("Add", $pinAccess)): ?>
                                            <a class="btn btn-secondary me-1 package-copy-btn"
                                               href="<?= $redirectPage ?>?act=<?= $act_1 ?>&copy_from=<?= (int) $row['id'] ?>"
