...
if (!function_exists('sorFetchSpxTrackingStatus')) {
     ...
     */
    function sorFetchSpxTrackingStatus($trackingNo)
    {
        $trackingNo = strtoupper(trim((string) $trackingNo));
        ...
        if (!is_array($response) || (isset($response['retcode']) && (int) $response['retcode'] !== 0)) {
            return '';
        }
    ...
    function sorRefreshTrackingStatus($financeConnect, $requestId, &$message = '', $cmsConnect = null)
    {
        ...
        if (sorIsSpxTrackingNumber($trackingNo)) {
            ...
            if ($statusText === '' && $canUseSpxFallback) {
                $statusText = sorFetchSpxTrackingStatus($trackingNo);
            }
        }
        ...
        if ((!$statusIsUsable || $statusIsError) && $trackingNo !== '') {
            ...
            if ($wsStatus !== '') {
                ...
                if (($statusText === '' || stripos($statusText, 'Unable to retrieve') !== false)
                    && $canUseSpxFallback) {
                    $spxStatus = sorFetchSpxTrackingStatus($trackingNo);
                    if ($spxStatus !== '') {
