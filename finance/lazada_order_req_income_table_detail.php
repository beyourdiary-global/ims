            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()) {
                    ...
                    <tr>
                        ...
                        <a class="btn btn-danger" onclick="confirmationDialog(<?= json_encode((string) $row['id']) ?>,[<?= json_encode((string) $row['curr_unit']) ?>,<?= json_encode((string) $row['country']) ?>],<?= json_encode((string) $pageTitle) ?>,<?= json_encode((string) $redirectPage) ?>,<?= json_encode((string) $deleteRedirectPage) ?>,'D')"><i class="fas fa-trash-alt"></i></a>
                        <?php endif; ?>
                        ...
                        <td scope="row"><?= htmlspecialchars((string) $row['curr_unit'], ENT_QUOTES, 'UTF-8') ?></td>
                        <td scope="row"><?= htmlspecialchars((string) $row['country'], ENT_QUOTES, 'UTF-8') ?></td>
