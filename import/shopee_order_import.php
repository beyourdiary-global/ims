...
$sorCustomerNameColumnExists = function_exists('shopeeOmsTableHasColumn')
    ? shopeeOmsTableHasColumn($finance_connect, dbFinance, SHOPEE_SG_ORDER_REQ, 'customer_name')
    : false;
...
if ($action === 'parseShopeeOrderReq') { // Shopee Order HTML/PDF Parsing
    ...
    if (!isset($_FILES['import_file'])) {
        ...
        if (!empty($importErrors)) {
            ...
            $detectedSkuList = array();
            if ($extension === 'pdf') {
                $skuSourceText = $pdfSourceText !== '' ? $pdfSourceText : $cleanText;
                $detectedSkuList = extractShopeeSkuCandidatesFromText($skuSourceText);
                if (empty($detectedSkuList)) {
                    $detectedSkuList = extractShopeeSkuCandidatesFromText($cleanText);
                }
                if (empty($detectedSkuList)) {
                    $detectedSkuList = extractShopeeSkuCandidatesFromPdfBinary((string) $rawContent);
                }
                ...
                $detectedSkuList = extractShopeeSkuCandidatesFromHtml($xpath);
            }
            $sku = formatDetectedImportSkuList($detectedSkuList);

            // Extract one or more product names for package matching.
            $productNameSourceText = ($extension === 'pdf' && $pdfSourceText !== '') ? $pdfSourceText : $cleanText;
            $productNameCandidates = extractShopeeProductNameCandidates($xpath, $productNameSourceText);
            ...
            $skuMatched = (!empty($pkgMatchInfo['matched_skus']) || (!empty($pkgIds) && $sku === '')) ? 'yes' : 'no';
            $previewData = [
                'order_id' => $order_id,
                'sku' => $sku,
                'detected_sku' => $sku,
    ...
    $postedDetectedSkuList = normalizeDetectedImportSkuList(post('detected_sku'));
    $postedPackageIdList = array_filter(array_map('trim', explode(',', (string) $packageIdsStr)), 'strlen');
    ...
    $previewData = [
        ...
        'sku' => formatDetectedImportSkuList(post('sku')),
        'detected_sku' => formatDetectedImportSkuList(post('detected_sku')),
        'sku_matched' => postSpaceFilter('sku_matched') === 'yes' ? 'yes' : 'no',
        'missing_sku' => !empty($postedDetectedSkuList)
            ? count($postedPackageIdList) < count($postedDetectedSkuList)
            : empty($packageIdsStr),
...
function extractShopeeHtmlIncomeAmountByLabels($html, $labels)
{
    ...
    foreach ((array) $labels as $label) {
        ...
        $pattern = '/income-label-text[^>]*>\s*' . $labelPattern . '\s*(?:<!--.*?-->\s*)?<\/div>.*?income-value[^>]*>.*?([\-]?\s*(?:RM|MYR|SGD|USD|S\$|\$)\s*[0-9][0-9,]*\.[0-9]{2})/is';
...
function extractShopeeProductNameCandidates($xpath, $cleanText)
{
    ...
    $sourceText = (string) $cleanText;
    $lines = getPdfTextLines($sourceText);
    ...
    for ($i = 0; $i < $lineCount; $i++) {
        ...
        if (preg_match('/\bProduct\(s\)\b/i', $line) !== 1) {
            continue;
        }

        $productParts = array();
        for ($j = $i + 1; $j < min($lineCount, $i + 12); $j++) {
            $candidateLine = trim((string) $lines[$j]);
            if ($candidateLine === '') {
                continue;
            }
            if (preg_match('/\b(?:Variation|See\s+Income\s+Details|Merchandise\s+Subtotal|Estimated\s+Shipping\s+Subtotal|Fees\s*&\s*Charges|Estimated\s+Order\s+Income|Order\s+Adjustment|Final\s+Amount|Buyer\s+Payment|Payment\s+Information|View\s+transaction\s+history)\b/i', $candidateLine)) {
                break;
            }
            if (preg_match('/^\d+$/', $candidateLine)) {
                continue;
            }

            $candidateLine = preg_replace('/\s+(?:RM|MYR|SGD|USD|S\$|\$)?\s*\d[\d,]*\.\d{2}\s+\d+\s+(?:RM|MYR|SGD|USD|S\$|\$)?\s*\d[\d,]*\.\d{2}\s*$/i', '', $candidateLine);
            $candidateLine = preg_replace('/\s+(?:RM|MYR|SGD|USD|S\$|\$)?\s*\d[\d,]*\.\d{2}\s*$/i', '', $candidateLine);
            $candidateLine = normalizeImportText($candidateLine);
            if ($candidateLine !== '') {
                $productParts[] = $candidateLine;
            }
        }

        if (!empty($productParts)) {
            $productName = normalizeImportText(implode(' ', $productParts));
            $productName = preg_replace('/\b(?:Unit\s+Price|Quantity|Subtotal|No\.)\b/i', ' ', $productName);
            $productName = normalizeImportText($productName);
            if ($productName !== '') {
    ...
    if (preg_match_all('/(?:Product\(s\)|Product Name|Item)\s*[:\-]?\s*(.+?)(?:SKU|Item Code|Variation|Qty|Quantity|Payment|Order|$)/is', normalizeImportText($sourceText), $matches)) {
        foreach ($matches[1] as $match) {
            $name = normalizeImportText($match);
            $name = preg_replace('/\b(?:Unit\s+Price|Quantity|Subtotal|No\.)\b/i', ' ', $name);
            $name = preg_replace('/\s+(?:RM|MYR|SGD|USD|S\$|\$)?\s*\d[\d,]*\.\d{2}(?:\s+\d+\s+(?:RM|MYR|SGD|USD|S\$|\$)?\s*\d[\d,]*\.\d{2})?.*$/i', '', $name);
...
function normalizeDetectedImportSkuList($skuValues)
{
    $normalized = array();

    foreach ((array) $skuValues as $skuValue) {
        if (!is_scalar($skuValue)) {
            continue;
        }

        $parts = preg_split('/[\r\n,]+/', (string) $skuValue);
        if ($parts === false) {
            $parts = array((string) $skuValue);
        }

        foreach ($parts as $part) {
            $candidate = normalizeShopeeSkuCandidate($part);
...
function formatDetectedImportSkuList($skuValues)
{
...
function collectShopeeDetectedSkuCandidate($rawCandidate, &$detectedSkuMap)
{
    ...
    if ($score < 5 || isset($detectedSkuMap[$candidate])) {
        return;
    }

    $detectedSkuMap[$candidate] = $candidate;
}

function extractShopeeSkuCandidatesFromHtml($xpath)
{
    if (!($xpath instanceof DOMXPath)) {
        return array();
    }

    $detectedSkuMap = array();
    $queries = array(
    ...
    return array_values($detectedSkuMap);
}

function extractShopeeSkuFromHtml($xpath)
{
    $detectedSkuList = extractShopeeSkuCandidatesFromHtml($xpath);
    return !empty($detectedSkuList) ? $detectedSkuList[0] : '';
}

function resolvePackageMatchesFromDetectedData($detectedSkus, $productNameCandidates, $connect, $packageMetaOptions = array())
{
    $resolvedIds = array();
    $matchedSkus = array();
    $unmatchedSkus = array();
    ...
    $matchIdsFromMeta = function ($value, $preferItemCode = false) use ($packageMetaOptions) {
        ...
        if ($preferItemCode && !empty($matchedIds)) {
            return $matchedIds;
        }
        ...
        if (!$preferItemCode && strlen($normalizedValue) >= 8) {
            foreach ($packageMetaOptions as $pkgId => $pkgMeta) {
                $packageNameLookup = normalizeImportLookup(isset($pkgMeta['name']) ? $pkgMeta['name'] : '');
                if (strlen($packageNameLookup) < 8) {
                    continue;
                }
                if (strpos($normalizedValue, $packageNameLookup) !== false || strpos($packageNameLookup, $normalizedValue) !== false) {
                    $pushMatchedId($pkgId);
                }
            }
        }
    ...
    $matchIdsFromDatabase = function ($value) use ($connect) {
        $value = trim((string) $value);
        $matchedIds = array();
        if ($value === '') {
            return $matchedIds;
        ...
        $pkgResult = getData('id', "item_code='$safeValue' OR name='$safeValue'", 'LIMIT 1', PKG, $connect);
        if ($pkgResult && $pkgResult->num_rows > 0) {
            $pkgRow = $pkgResult->fetch_assoc();
            $pushMatchedId($pkgRow['id']);
            return $matchedIds;
        }

        $safeValueLike = sanitizeImportLikeTerm($value);
        if ($safeValueLike === '') {
            return $matchedIds;
        }

        $escapedValueLike = mysqli_real_escape_string($connect, $safeValueLike);
        try {
            $pkgResult = getData('id', "item_code LIKE '%$escapedValueLike%' OR name LIKE '%$escapedValueLike%'", 'LIMIT 1', PKG, $connect);
    ...
    foreach (normalizeDetectedImportSkuList($detectedSkus) as $sku) {
        $matchedIds = $matchIdsFromMeta($sku, true);
        if (empty($matchedIds)) {
            $matchedIds = $matchIdsFromDatabase($sku);
        }
    ...
    foreach ($productNameCandidates as $productName) {
        ...
        $matchedIds = $matchIdsFromMeta($productName);
        if (!empty($matchedIds)) {
            foreach ($matchedIds as $matchedId) {
                $pushId($matchedId);
            }
        ...
        // Create an ASCII-safe lookup term to avoid collation issues on legacy package columns.
...
{
    $matchInfo = resolvePackageMatchesFromDetectedData($sku, $productNameCandidates, $connect, $packageMetaOptions);
    return isset($matchInfo['package_ids']) ? (array) $matchInfo['package_ids'] : array();
}
...
function normalizeShopeeSkuCandidate($value)
{
    ...
    // Shopee order pages can append a nearby CTA like "See Income Details" directly after the SKU text.
    $value = preg_replace('/SEE\s*INCOME\s*DETAILS.*$/i', '', $value);
...
function extractShopeeSkuCandidatesFromPdfBinary($rawContent)
{
...
{
    $detectedSkuList = extractShopeeSkuCandidatesFromPdfBinary($rawContent);
    return !empty($detectedSkuList) ? $detectedSkuList[0] : '';
}

function extractShopeeSkuCandidatesFromText($text)
{
    $text = (string) $text;
    if ($text === '') {
        return array();
...
{
    $detectedSkuList = extractShopeeSkuCandidatesFromText($text);
    return !empty($detectedSkuList) ? $detectedSkuList[0] : '';
}
...
function normalizeImportAmount($rawAmount)
{
    ...
    $value = str_replace(['RM', 'MYR', 'USD', 'SGD', 'S$', '$'], '', strtoupper($value));
...
function extractImportAmountsFromText($text)
{
    ...
    $results = array();
    if (preg_match_all('/-?\s*(?:RM|MYR|SGD|USD|S\$|\$)?\s*[0-9][0-9,]*\.[0-9]{2}/i', $text, $matches)) {
...
function extractShopeePdfOcrServiceFeeDirect($text)
{
    ...
    if (preg_match('/SERVICE\s*FEE.{0,100}?(?:RM|MYR|SGD|USD|S\$|\$)?\s*(0|00|000|0000|0\.00)(?![0-9])/u', $text) === 1) {
        return '0.00';
    }
...
function extractShopeePdfAmountByLooseLabels($text, $labels)
{
    ...
    foreach ((array) $labels as $label) {
        ...
        $pattern = '/' . $labelPattern . '.{0,220}?(-?\s*(?:RM|MYR|SGD|USD|S\$|\$)?\s*[0-9][0-9,]*\.[0-9]{2})/i';
...
function extractShopeePdfVoucherAmount($text)
{
    ...
    if (preg_match('/' . $labelPattern . '.{0,180}?-\s*(?:RM|MYR|SGD|USD|S\$|\$)?\s*([0-9][0-9,]*\.[0-9]{2})/i', $normalizedText, $matches)) {
    ...
    for ($i = 0; $i < $lineCount; $i++) {
        ...
        if (preg_match_all('/-\s*(?:RM|MYR|SGD|USD|S\$|\$)?\s*([0-9][0-9,]*\.[0-9]{2})/i', $nearText, $matches) && !empty($matches[1])) {
...
function parseShopeeOrderAmountByLabels($text, $labels)
{
    ...
    foreach ($labels as $label) {
        $currencyPattern = '/' . preg_quote($label, '/') . '.{0,220}?(-?\s*(?:RM|MYR|SGD|USD|S\$|\$)\s*[0-9][0-9,]*\.?[0-9]*)(?!\s*%)/i';
        ...
        if ($looseLabelPattern !== '') {
            $looseCurrencyPattern = '/' . $looseLabelPattern . '.{0,320}?(-?\s*(?:RM|MYR|SGD|USD|S\$|\$)\s*[0-9][0-9,]*\.?[0-9]*)(?!\s*%)/i';
...
function parseShopeeOrderAmountToken($text)
{
    ...
    if (preg_match_all('/-?\s*(?:RM|MYR|SGD|USD|S\$|\$)\s*[0-9][0-9,]*\.?[0-9]*(?!\s*%)/i', $normalized, $matches) && !empty($matches[0])) {
...
function extractShopeeAmountNearLabel($text, $label)
{
    ...
    if (preg_match('/-?\s*(?:RM|MYR|SGD|USD|S\$|\$)\s*[0-9][0-9,]*\.?[0-9]*(?!\s*%)/i', $afterLabel, $matches)) {
...
function detectShopeeOrderCurrency($text)
{
    ...
    if (preg_match('/(?:\bSGD\s*[0-9]|S\$\s*[0-9])/i', $text)) {
        return 'SGD';
    }
    if (preg_match('/\bUSD\s*[0-9]/i', $text)) {
        return 'USD';
    }
    if (preg_match('/\$\s*[0-9]/', $text) && preg_match('/(?:urbanism\.candy\.sg|\.sg\b|SPXSG|\bSG\b|Order\s*ID\s+[A-Z0-9]{8,}SG\b)/i', $text)) {
        return 'SGD';
    }
                                ...
                                <form method="post" enctype="multipart/form-data" autocomplete="off" data-shopee-import-preview="1" novalidate>
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-12 col-md-4">
                                            ...
                                            <?php
                                            $detectedSkuPreviewList = normalizeDetectedImportSkuList(isset($previewData['detected_sku']) ? $previewData['detected_sku'] : '');
                                            $detectedSkuPreviewLabel = count($detectedSkuPreviewList) > 1 ? 'Detected SKUs' : 'Detected SKU';
                                            ?>
                                            ...
                                            <?php if (!empty($previewData['detected_sku'])) { ?>
                                                <small class="<?= $previewData['missing_sku'] ? 'text-muted' : 'text-success' ?>">
                                                    <?= htmlspecialchars($detectedSkuPreviewLabel) ?>: <?= htmlspecialchars($previewData['detected_sku']) ?><?= $previewData['missing_sku'] ? '' : ' - package auto-selected.' ?>
                                                </small>
                                            <?php } else { ?>
                                        ...
                                        <div class="col-12 col-md-4">
                                            ...
                                            $matchedPackagePreviewNames = array_values(array_filter(array_map(function ($pkgRow) {
                                                return trim((string) (isset($pkgRow['name']) ? $pkgRow['name'] : ''));
                                            }, $pkgRows), 'strlen'));
                                            ?>
                                            ...
                                            <?php if ($previewData['missing_sku'] && !empty($matchedPackagePreviewNames)) { ?>
                                                <div class="text-warning fw-bold mb-1" style="font-size:12px;"><i class="fa-solid fa-circle-exclamation"></i> Partial auto-match: <?= htmlspecialchars(implode(', ', $matchedPackagePreviewNames)) ?>. Please verify and add the remaining package manually.</div>
                                            <?php } else if ($previewData['missing_sku']) { ?>
                                            ...
                                            <?php } else if (!empty($previewData['sku'])) { ?>
                                                <div class="text-success fw-bold mb-1" style="font-size:12px;"><i class="fa-solid fa-circle-check"></i> Auto-matched package(s) from detected SKU(s).</div>
                                            <?php } ?>
                                    ...
                                    <div class="row mb-3 shopee-airbill-row">
                                        ...
                                        <div class="col-12 col-md-6">
                                            ...
                                            <input type="hidden" id="customer_name" name="customer_name" value="<?= htmlspecialchars(isset($previewData['customer_name']) ? $previewData['customer_name'] : '', ENT_QUOTES, 'UTF-8') ?>">
                                        </div>
                                    </div>
                                                ...
                                                <?php if ($previewAttachmentSrc !== '') { ?>
                                                    <div id="airbill_attachment_preview_wrap" class="shopee-airbill-preview-media">
                                                        <?php if (in_array($previewAttachmentExt, array('png', 'jpg', 'jpeg', 'webp', 'gif', 'svg'), true)) { ?>
                                                            <img id="airbill_attachment_preview_img"
                                                                 src="<?= htmlspecialchars($previewAttachmentSrc, ENT_QUOTES, 'UTF-8') ?>"
                                                                 alt="Airbill Attachment Preview">
                                                        <?php } else if ($previewAttachmentExt === 'pdf') { ?>
                                                            <iframe id="airbill_attachment_preview_pdf"
                                                                    src="<?= htmlspecialchars($previewAttachmentSrc, ENT_QUOTES, 'UTF-8') ?>"
...
<script>
    (function syncShopeeImportPreviewForm() {
        ...
        var currentAirbillPreviewObjectUrl = null;

        function clearAirbillPreviewObjectUrl() {
            if (currentAirbillPreviewObjectUrl) {
                URL.revokeObjectURL(currentAirbillPreviewObjectUrl);
                currentAirbillPreviewObjectUrl = null;
            }
        }

        if (airbillAttachmentInput && airbillAttachmentPreviewWrap) {
            airbillAttachmentInput.addEventListener('change', function () {
                clearInlineError(this);

                var file = airbillAttachmentInput.files && airbillAttachmentInput.files[0] ? airbillAttachmentInput.files[0] : null;
                if (!file) {
                    clearAirbillPreviewObjectUrl();
                    airbillAttachmentPreviewWrap.innerHTML = '';
                    airbillAttachmentPreviewWrap.style.display = 'none';
                    return;
                }
                ...
                currentAirbillPreviewObjectUrl = fileUrl;

                airbillAttachmentPreviewWrap.style.display = 'block';

                if (file.type.indexOf('image/') === 0) {
                    airbillAttachmentPreviewWrap.innerHTML =
                        '<img id="airbill_attachment_preview_img" src="' + fileUrl + '" alt="Airbill Attachment Preview">';
                } else if (file.type === 'application/pdf' || fileName.endsWith('.pdf')) {
                    airbillAttachmentPreviewWrap.innerHTML =
                        '<iframe id="airbill_attachment_preview_pdf" src="' + fileUrl + '" title="Airbill Attachment Preview"></iframe>';
                } else {
                    clearAirbillPreviewObjectUrl();
                    airbillAttachmentPreviewWrap.innerHTML = '';
                    airbillAttachmentPreviewWrap.style.display = 'none';
